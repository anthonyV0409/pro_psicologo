...
def registrocita():
    if request.method == 'POST':
        ...
        sqlInsertar = "insert into citas( id_cliente, id_psicologo,fecha,estado)values(%s,%s,%s,%s)"
        cursor.execute(sqlInsertar, (cedula, cita, fechac,'no aceptado'))
...
def datoscitasp():
    ...
    return render_template('citaspsicologo.html', datoscitapsico=usuario, verpcli=pssl)
